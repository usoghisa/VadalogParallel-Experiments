...
import prometheuxresearch.benchmark.vldb.experiment.flink.psc.CoGroupDeduplicationFunction;
import prometheuxresearch.benchmark.vldb.experiment.flink.psc.ReasoningKeyMapper;
import prometheuxresearch.benchmark.vldb.experiment.flink.psc.ReasoningKeyMapper2;
...
public class StrongLinkFlink {
	...
	public static long run(ParameterTool params) throws Exception {
		...
		DataSet<Tuple3<String, String, String>> slApp = psc.join(psc).where(1, 2).equalTo(1, 2).projectFirst(0, 1)
				.projectSecond(0);

		DataSet<Tuple4<String, String, String, Integer>> strongLink = slApp.filter(t -> !t.f0.equals(t.f2))
				.map(new MapFunction1()).groupBy(0, 1, 2).sum(3).filter(t -> t.f3 > 3);

		long countSL = strongLink.count();

		System.out.println("The number of strong link relationships found is " + countSL);
		...
		if (execMode.equals("test")) {
			...
			strongLink.writeAsCsv(outputPath);
