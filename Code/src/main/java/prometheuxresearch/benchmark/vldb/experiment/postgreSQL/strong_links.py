...
def init_tables(connection):
    ...
    try:
        ...
        cursor.execute("""
                       CREATE INDEX StrongLinkIndex ON Psc (company2);
                       """)
...
def count_strong_link(connection):
    cursor = None
    try:
        query = """
              SELECT COUNT(*)
...
def run_final_rule(connection):
    try:
        query = """
                INSERT INTO StrongLink(company1, company2, nPerson, companyFrom)
                SELECT t1.company1 AS company1, t2.company1 AS company2, 
                COUNT(*) AS nPerson, t1.company2 AS companyFrom
                FROM Psc AS t1 JOIN Psc AS t2 ON t1.company2 = t2.company2 AND t1.person = t2.person
                GROUP BY (t1.company1, t2.company1, t1.company2)
                HAVING t1.company1 <> t2.company1 AND COUNT(*) > 3;
                """
        run_query_and_commit(connection, query)

    except psycopg2.Error as e:
        print("Error running final TGD.", e)
...
def drop_all_tables_and_views(connection):
    try:
        ...
        cursor.execute("""DROP TABLE IF EXISTS StrongLink CASCADE;""")
...
if connection:
    ...
    try:
        ...
        print("Computing Strong links...")
        run_final_rule(connection)
        n_strong_link = count_strong_link(connection)
        ...
        print("Number of strong link found: " + str(n_strong_link))
